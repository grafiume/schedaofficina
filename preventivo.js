(function(){
  ...
  function bindUI(){
    ...
    $('btnOpenRecord')?.addEventListener('click', ()=>{
      if(record?.id) location.href=`record.html?id=${encodeURIComponent(record.id)}`;
    });
  ...
  function renderDirtyState(){
    ...
    $('dirtyState').className='small '+(dirty?'text-danger':'text-muted');
  }
  ...
  function renderQuoteHeader(){
    ...
    $('quoteStatusBadge').className='badge '+(
      st==='ACCETTATO' ? 'bg-success' :
      st==='INVIATO' ? 'bg-primary' :
      st==='ANNULLATO' ? 'bg-danger' :
      'bg-secondary'
    );
    ...
    $('dueLabel').className='small mt-1 '+d.cls;
  ...
  function renderTasks(){
    ...
    WORKS.forEach((w, idx)=>{
      ...
      tr.className=`${checked?`row-${meta.v==='COMPLETATA'?'complete':meta.v==='IN_LAVORAZIONE'?'working':'todo'}`:'row-off'} ${quoteState.is_urgent?'row-urgent':''}`;
      ...
      tr.innerHTML=`
        ...
        <td class="rip-main">
          <div class="rip-title">
            ...
            ${w.free?`<input data-editable="1" class="form-control free-desc mt-1" placeholder="Descrizione lavorazione libera…" value="${esc(item?.description||'')}" ${checked?'':'disabled'}>`:esc(w.text)}
          </div>
          <div class="rip-meta">
            <div class="meta-box">
        ...
        <td class="line-progress-wrap">
          <div class="linebar">
            <div class="st-${meta.v}" style="width:${checked?Math.max(5,pct):0}%"></div>
          </div>
          <div class="small text-muted mt-1">${checked?`${pct}%`:''}</div>
        </td>
      `;
      ...
      price.addEventListener('blur', ()=>{
        if(quoteState.items[w.code]) price.value=fmtMoney(quoteState.items[w.code].unit_price_ex_vat);
      });
  ...
  function recalcTotals(){
    ...
    $('quoteProgBar').style.width=`${Math.max(0, Math.min(100, prog))}%`;
  }
  ...
  async function deleteQuote(){
    ...
    try{
      const { error: e1 } = await sb.from('quote_items').delete().eq('quote_id', currentQuoteId);
      if(e1) throw e1;

      const { error: e2 } = await sb.from('quotes').delete().eq('id', currentQuoteId);
      if(e2) throw e2;
      ...
      setTimeout(()=>{
        if(recId) location.href='preventivo.html?record_id='+encodeURIComponent(recId);
        else history.back();
      }, 500);
    }catch(e){
  ...
  async function saveAll(){
    ...
    try{
      ...
      if(!currentQuoteId){
        const { data, error } = await sb.from('quotes').insert(qPayload).select().single();
        if(error) throw error;
        currentQuoteId=data.id;
        quoteState.id=data.id;
        try{
          const u=new URL(location.href);
          u.searchParams.delete('record_id');
        ...
        const { error } = await sb.from('quotes').update(qPayload).eq('id', currentQuoteId);
        if(error) throw error;
      }

      const existingIds=Object.values(originalState?.items||{}).map(x=>x.id).filter(Boolean);

      const currentItems=WORKS.map((w,idx)=>{
      ...
      if(deleteIds.length){
        const { error } = await sb.from('quote_items').delete().in('id', deleteIds);
        if(error) throw error;
      }

      for(const item of currentItems){
        if(item.id){
          const { error } = await sb.from('quote_items').update(item).eq('id', item.id);
          if(error) throw error;
        } else {
          delete item.id;
          const { data, error } = await sb.from('quote_items').insert(item).select().single();
          if(error) throw error;
          quoteState.items[data.rip_code||item.rip_code].id=data.id;
        }
      }
