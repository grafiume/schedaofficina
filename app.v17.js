...
function openLightbox(url){
  ...
  if (!_lbModal){
    ...
    btn.onclick = ()=>{
      ...
      if (_lensActive){ _lensEl && (_lensEl.style.display='block'); }
      else { _lensEl && (_lensEl.style.display='none'); }
    };
  }
  _lbModal.show();
  img.onload = ()=>{
    setupLens(img);
    // start with lens ON by default
    _lensActive = true;
...
function setupLens(img){
  if (!_lensEl){
    ...
    img.closest('.modal-body').appendChild(_lensEl);
  }
  ...
  img.onmousemove = update;
  img.ontouchmove = (ev)=>{ if(ev.touches&&ev.touches[0]) update(ev.touches[0]); };
}