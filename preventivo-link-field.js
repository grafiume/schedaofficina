...
(function () {
  ...
  function getParam(names) {
    try {
      ...
      for (const n of names) { const v = p.get(n); if (v) return v; }
    } catch {}
    return null;
  }
  ...
  function detectRecordId() {
    ...
    const sel = [
      'input[name="id"]','input#id','input#record-id','[data-record-id]',
      'input[name="record_id"]','input[name="scheda_id"]'
    ];
    for (const s of sel) {
      const el = document.querySelector(s);
      if (el) {
        const val = el.getAttribute('data-record-id') || el.value;
        if (val) return String(val);
      }
    }
    const q = getParam(['id','schedaId','record','rid']);
    if (q) return String(q);
    return null;
  }

  function findMountPoint() {
    const h = Array.from(document.querySelectorAll('h2,h3,legend'))
      .find(x => /scheda.*cliente/i.test(x.textContent||""));
    if (h && h.parentElement) return h.parentElement;
    const lab = Array.from(document.querySelectorAll('label'))
      .find(x => /preventivo/i.test(x.textContent||""));
  ...
  save.onclick = async function () {
    ...
    if (!isValid(url)) { alert('Inserisci un link valido con ?pvid=UUID'); return; }
    if (!sb) { alert('Client Supabase non trovato in pagina'); return; }

    // 👇 se non ho l'ID, LO CHIEDO ADESSO
    if (!recordId) {
      const manual = prompt("ID scheda non rilevato. Incolla l'UUID della scheda:");
      if (!manual) return;
      recordId = manual.trim();
    }
