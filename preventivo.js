(function(){
  ...
  function renderEditState(){
    ...
    ['status','sent_at','accepted_at','delivery_days','delivery_date','notes','is_urgent'].forEach(id=>{ const el=$(id); if(!el) return; el.disabled=!isEditUnlocked; });
    document.querySelectorAll('[data-editable="1"]').forEach(el=>{ el.disabled=!isEditUnlocked; });
  }
  function touch(){ recalcTotals(); renderQuoteHeader(); renderDirtyState(); }
  function renderDirtyState(){ const dirty=isDirty(); $('dirtyState').textContent=dirty?'Modifiche non salvate':'Salvato'; $('dirtyState').className='label '+(dirty?'text-danger':''); }
  function renderAll(){ renderQuoteHeader(); renderTasks(); renderEditState(); renderDirtyState(); }
  function renderQuoteHeader(){
    $('recCliente').textContent=record?.cliente||'—'; $('recDesc').textContent=record?.descrizione||'—'; $('recModel').textContent=record?.modello?`Modello: ${record.modello}`:''; $('quoteId').textContent=quoteState?.id||'NUOVO (non ancora salvato)';
    const st=quoteState?.status||'BOZZA'; $('status').value=st;
    const statusBadge = $('quoteStatusBadge');
    if(statusBadge){
      statusBadge.textContent=st;
      statusBadge.className='top-pill '+(st==='ACCETTATO'?'green':st==='INVIATO'?'blue':st==='ANNULLATO'?'red':'gray');
    }
    $('sent_at').value=quoteState?.sent_at||''; $('accepted_at').value=quoteState?.accepted_at||''; $('delivery_days').value=quoteState?.delivery_days??''; $('delivery_date').value=quoteState?.delivery_date||''; $('notes').value=quoteState?.notes||'';
    const urgentEl=$('is_urgent'); if(urgentEl) urgentEl.checked=!!quoteState?.is_urgent;
    const d=computeDueInfo(quoteState||{}); $('dueLabel').textContent=d.text; $('dueLabel').className='due-banner '+d.cls;
    const miniTop=$('urgentMiniBannerTop'); if(miniTop){ miniTop.style.display=quoteState?.is_urgent ? 'inline-flex' : 'none'; }
  }
        ...
        <td colspan="5">
          <div class="row-inner">
            ...
            <div class="status-stack">
              <select data-editable="1" class="form-select status-select row-status" ${checked?'':'disabled'}>${STATUS.map(s=>`<option value="${esc(s.v)}" ${(item?.work_status||'DA_FARE')===s.v?'selected':''}>${esc(s.label)}</option>`).join('')}</select>
            </div>
  ...
  async function deleteQuote(){
    ...
    const ok = confirm('Vuoi cancellare definitivamente questo preventivo?'); if(!ok) return;
    clearErr();
    try{
      ...
      currentQuoteId = null; quoteState = emptyQuoteState(recId); originalState = clone(quoteState); renderAll();
