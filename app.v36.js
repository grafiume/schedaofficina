...
// --- runtime self-checks (dev safety) ---
(function(){
  try{
    const code = document.currentScript && document.currentScript.text || '';
    if (/^\s*\/\s*---/m.test(code)) throw new Error('Found malformed comment "/ ---" in script');
    if ((code.match(/\bfunction\s+openLightbox\s*\(/g)||[]).length !== 1) throw new Error('openLightbox defined != 1');
  }catch(e){
...
// --- In-page overlay image viewer (single source of truth) ---
