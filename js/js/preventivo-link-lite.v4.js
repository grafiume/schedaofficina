...
/* preventivo-link-lite.v4.js — per-record + iOS fallback redirect */
(function(){
  ...
  // Path fisso su GitHub Pages (stesso repo): cambialo solo se la cartella non è /schedaofficina
  ...
  // --- utils
  const PVID_RE = /^https:\/\/grafiume\.github\.io\/preventivi-elip\/\?pvid=[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  ...
  // iOS fallback: usa una pagina di redirect sullo stesso dominio per preservare SEMPRE la query ?pvid=...
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
  btnO.addEventListener('click', function(ev){
    const href = normalize(input.value||"");
    if (!isUrl(href)) return;
    if (isIOS && PVID_RE.test(href)){
      ev.preventDefault();
      // Costruisci URL verso redirect.html nello stesso dominio
      const redir = `${location.origin}${REDIRECT_PATH}?to=${encodeURIComponent(href)}`;
      // apri in stessa tab per evitare che iOS perda la query
      location.href = redir;
    }
  });
