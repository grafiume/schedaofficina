...
async function refreshGallery(recordId){
  ...
  paths.forEach(p=>{
    ...
    img.addEventListener('click', ()=>openLightbox(img.src)); gallery.appendChild(col);
  });
}
const state = {
  all: [],
...
rows.sort(byHomeOrder).forEach(r=>{
  ...
  listPhotos(r.id).then(paths=>{
    ...
    if (paths.length){ const url = publicUrl(paths[0]); img.src = url; img.addEventListener('click', ()=>openLightbox(url));
      img.alt = 'Anteprima';
    } else {
      // no link in this build
      img.alt = '—';
    }
  }).catch(()=>{});
});
