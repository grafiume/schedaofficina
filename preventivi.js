(function(){
  ...
  function computeDue(q){
    ...
    if(!expected || isNaN(expected.getTime())) return {label:'—', sub:'', urgent:false};
    const diff=Math.round((expected.getTime()-now.getTime())/(1000*60*60*24));
    if(diff>0) return {label:`${q.is_urgent?'URGENTE • ':''}mancano ${diff} gg`, sub:fmtDate(expected), urgent:!!q.is_urgent};
    if(diff<0) return {label:`${q.is_urgent?'URGENTE • ':''}ritardo ${Math.abs(diff)} gg`, sub:fmtDate(expected), urgent:true};
    return {label:`${q.is_urgent?'URGENTE • ':''}scade oggi`, sub:fmtDate(expected), urgent:!!q.is_urgent};
  }
  ...
  function isMeaningfulQuote(q){ return (q.item_count||0) > 0 || Number(q.subtotal_ex_vat||0) > 0 || !!q.notes || !!q.sent_at || !!q.accepted_at || !!q.delivery_days || !!q.delivery_date || !!q.is_urgent || ((q.status||'') !== 'BOZZA'); }
  let sb;
  async function load(){
    clearErr();
    ...
    try{
      ...
      render(rows);
    }catch(e){ showErr('Errore caricamento preventivi: ' + (e?.message||e)); if(tbody) tbody.innerHTML='<tr><td colspan="7" class="text-center py-4 text-muted">Nessun dato</td></tr>'; }
  }
